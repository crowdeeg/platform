...
$.widget("crowdeeg.TimeSeriesAnnotator", {
		...
		sources.forEach(function (source) {
			for (var prop in source) {
				if (
					...
					target[prop] = that._mergeObjectsDeeply(
						target[prop],
						source[prop]
					);
				} else {
	...
	_showConsentForm: function () {
		...
		if (
			that.options.showConfirmationCode &&
			that.options.confirmationCode
		) {
	...
	_setVisibilityStatusForInfoPanel: function (isVisible) {
		...
		const setVisibilityStatus =
			that.options.setVisibilityStatusForInfoPanel;
	...
	_isCurrentWindowSpecifiedTrainingWindow: function () {
		...
		return (
			that.vars.currentTrainingWindowIndex <
	...
	_getSpecifiedTrainingWindows: function () {
		...
		for (f = 0; f < featureOrder.length; ++f) {
			...
			var featureTrainingWindows =
				featureOptions[feature].training.windows;
	...
	_isTrainingEnabled: function () {
		var that = this;
		return (
			that._isHITModeEnabled() &&
			that.options.visibleRegion.training.enabled
		);
	},
	...
	_setupExamplesMode: function () {
		...
		$(that.element)
			...
			.prepend(
				'<span class="examples-title pull-left">Examples for:</span>'
			);
		$(that.element)
			.find(".button_container")
			.append(
		...
		if (that.options.features.scrollThroughExamplesAutomatically) {
			$(that.element).hover(function () {
				if (that.vars.scrollThroughExamplesIntervalId !== undefined)
					return;
				that.vars.scrollThroughExamplesIntervalId = window.setInterval(
					function () {
						that._showNextExample(1);
					},
					that.options.features.scrollThroughExamplesSpeedInSeconds *
						1000
				);
			});
		}
	...
	_showNextExample: function (stepLength) {
		...
		do {
			...
			that.vars.currentExampleIndex %=
				that.options.features.examples.length;
	...
	_setupExperiment: function () {
		...
		$(that.element)
			.find(".experiment_container .hints_container")
			.append(hint);
	},
	...
	_updateNavigationStatusForExperiment: function () {
		...
		var conditionWindows =
			that.options.experiment.current_condition.windows;
		...
		that._setFastForwardEnabledStatus(
			windowsRemaining >= that.options.windowJumpSizeFastForwardBackward
		);
		...
		if (
			that.options.experiment.current_condition.temporal_context ==
			"shuffled"
		) {
	...
	_setupTimer: function () {
		...
		if (preferences.lastAnnotationTime) {
			that.vars.lastAnnotationTime = parseInt(
		...
		that._setTotalAnnotationTimeSeconds(
			...
			assign = Assignments.find(
				{
					...
					dataFiles: that.options.context.dataset.map(
						(data) => data._id
					),
				},
				{
	...
	_setupDownsampledRecording: function (that) {
		...
		return new Promise((resolve, reject) => {
			Meteor.call(
				...
				(error, results) => {
					if (error)
						throw new Error("Cannot downsample EDF file\n" + error);
					else resolve();
				}
			);
		});
	...
	_preloadEntireRecording: function (callback) {
		...
		if (montages) {
			montages.forEach(function (montage) {
				channelsDisplayedPerMontage.push(
					that._getChannelsDisplayed(montage)
				);
			});
		} else {
		...
		function updateLoadingProgress() {
			var percentage = Math.max(
				...
				Math.min(
					100,
					Math.round(
						(numWindowsLoaded / numWindowsToRequestTotal) * 100
					)
				)
			);
			...
			for (var i = 0; i < numWindowsToRequestPerMontage; ++i) {
				...
				var options = {
					...
					use_high_precision_sampling:
						that.options.useHighPrecisionSampling,
				};
				////console.log(options);
		...
		return new Promise((resolve, reject) => {
			...
			Meteor.call(
				...
				(error, results) => {
					if (error) {
						throw new Error(
							"Cannot get recording metadata\n" + error
						);
					}
					that.vars.recordingLengthInSeconds =
						results.lengthInSeconds;
	...
	_getUserStatus: function () {
		...
		if (that.options.experiment.running) {
			...
			that.options.allRecordings = [
				{
					_id: id,
					path: that.options.experiment.current_condition
						.recording_name,
				},
			];
			...
			if (
				...
				preferencesArbitrationRoundNumber <
					that._getArbitrationRoundNumberInt()
			) {
	...
	_toggleNoTimelockScroll: function (toggle) {
		...
		function scroll(e) {
			...
			if (channelIndex !== undefined) {
				let modifiedDataId =
					chart.series[channelIndex].options.custom.dataId;
	...
	_keyDownCallback: function (e) {
		...
		if (keyCode == 72) {
			...
			keyCode == 66 &&
			that.options.showBookmarkCurrentPageButton
		) {
			that._toggleBookmarkCurrentPage();
			return;
		} else if (
			...
			$(that.element)
				...
				.each(function () {
					...
					var characterKeyCodeLowerCase = character
						.toLowerCase()
						.charCodeAt(0);
	...
	_toggleClassificationSummary: function () {
		var that = this;
		var classificationSummaryElement =
	...
	_selectFeatureClass: function (featureClassButton) {
		...
		that.vars.activeFeatureType =
			featureClassButton.data("annotation-type");
	},
	...
	_shiftChart: function (windows) {
		...
		if (that.options.experiment.running) {
			...
			if (that._isCurrentWindowTrainingWindow()) {
				...
				nextRecordings = [
					{ _id: id, path: nextTrainingWindow.recordingName },
				];
				nextWindowStart = nextTrainingWindow.timeStart;
				nextWindowSizeInSeconds =
					nextTrainingWindow.windowSizeInSeconds;
			} else {
			...
			nextWindowStart = Math.max(
				0,
				that.vars.currentWindowStart +
					that.vars.xAxisScaleInSeconds * windows
			);
		}
		that._switchToWindow(
			nextRecordings,
	...
	_switchToWindow: function (allRecordings, start_time, window_length) {
		...
		if (!that._isCurrentWindowSpecifiedTrainingWindow()) {
			...
			if (that.options.visibleRegion.start !== undefined) {
				...
				start_time =
					window_length * Math.ceil(start_time / window_length);
			...
			if (that.options.visibleRegion.end !== undefined) {
				...
				start_time =
					window_length * Math.floor(start_time / window_length);
				var forwardEnabled =
					start_time + window_length <=
					that.options.visibleRegion.end - window_length;
		...
		if (
			that._isCurrentWindowFirstTrainingWindow() &&
			!that._isTrainingOnly()
		) {
			//console.log("4");
			bootbox
		...
		if (
			...
			for (
				var i = 1;
				i <= that.options.numberOfForwardWindowsToPrefetch;
				++i
			) {
					...
					if (that._isInNoTimelockMode()) {
						...
						switch (windowStartTime) {
							...
							case that.vars.currentWindowStart +
								window_length *
									that.options
										.windowJumpSizeFastForwardBackward:
								if (
									that.options.visibleRegion.end === undefined
								) {
									that._setFastForwardEnabledStatus(
	...
	_getProgressInPercent: function () {
		...
		var start =
			windowSize *
			Math.ceil(that.options.visibleRegion.start / windowSize);
		var end =
			windowSize *
			Math.floor(
				(that.options.visibleRegion.end - windowSize) / windowSize
			);
		if (!that._areTrainingWindowsSpecified()) {
			var progress =
			...
			var progress =
				(currentWindowIndex + 1) / (numberOfWindowsTotal + 2);
		}
	...
	_requestData: function (options, callback) {
		...
		var noDataError =
			"No data available for window with options " +
		...
		for (var dataId in input.channel_values) {
			...
			for (var name in input.channel_values[dataId]) {
                ...
                */
				...
				var numSamplesPaddedBefore =
					numSecondsPaddedBefore * samplingRate;
				...
				var numSamplesPaddedAfter =
					values.length -
					numSamplesPaddedBefore -
					numSamplesDataOfInterest;
		...
		data.channels.forEach((channel, c) => {
			...
			allFrequencyFilters.forEach((frequencyFilter) => {
				...
				filterNode.frequency.value =
					frequencyInHz * scaleFactorFrequency;
			...
			offlineCtx
				...
				.then((bufferFiltered) => {
					...
					var scaleFactorAmplitude =
						channel.audio.scaleFactors.amplitude;
					if (scaleFactorAmplitude != 0) {
	...
	_isDataValid: function (data) {
		...
		for (let dataId in data.channel_values) {
			if (Object.keys(data.channel_values[dataId]).length == 0)
				return false;
		}
		return true;
	},
        ...
        */
							...
							events: {
								click: function (e) {
									that._setCrosshair({
										...
										dataId: e.point.series.options.custom
											.dataId,
									});
								},
						...
						setExtremes: function (e) {
							if (e.trigger == "navigator") {
								var startTimeSnapped =
									Math.round(
										e.min / that.vars.xAxisScaleInSeconds
									) * that.vars.xAxisScaleInSeconds;
								startTimeSnapped = Math.max(
									0,
									startTimeSnapped
								);
		...
		if (that.options.features.examplesModeEnabled) {
			that._displayAnnotations(
				that._turnExamplesIntoAnnotations(
					that.options.features.examples
				)
			);
		}
		that._setupYAxisLinesAndLabels();
	...
	_setupAnnotationInteraction: function () {
		...
		if (!that.vars.setupOn) {
			...
			if (
				...
				!that.options.features.order.length
			)
				return;
			var chart = that.vars.chart;
			//    //console.log(chart);
		...
		that._saveAnnotation(
			...
			function (savedAnnotation, error) {
				if (savedAnnotation) {
					...
					annotationFormatted.arbitration =
						savedAnnotation.arbitration;
		...
		channels.forEach((channel, c) => {
			...
			referenceValues.forEach((referenceValue, r) => {
				var referenceValueWithGain =
					(referenceValue + offsetPreScale) *
					that._getGainForChannelIndex(c);
	...
	_getStaticFrequencyFiltersForChannel: function (channel) {
		var that = this;
		const inferredDataModality =
			that._inferDataModalityFromChannel(channel);
	...
	_isArbitrating: function () {
		var that = this;
		return (
			that._getArbitration() && that._getArbitrationRoundNumberInt() > 0
		);
	},
	...
	_getAnnotations: function (
		...
		annotations.forEach(function (annotation) {
			var annotationWindowStart =
				Math.floor(
					annotation.position.start / that.vars.xAxisScaleInSeconds
				) * that.vars.xAxisScaleInSeconds;
		...
		var visibleAnnotations = annotations.filter(function (annotation) {
			...
			if (annotationChannels) {
				...
				for (var c = 0; c < annotationChannels.length; ++c) {
					var channel =
						annotation.metadata.channels_displayed[
							annotationChannels[c]
						];
	...
	_displaySleepStageSelection: function (
		...
		if (
			preClassificationAnnotation &&
			preClassificationAnnotation.length > 0
		) {
		...
		try {
			annotations.forEach((annotation) => {
				if (
					that.vars.fullWindowLabels[annotation.label] ===
					"SLEEP_STAGE"
				) {
					var cacheKey = that._getAnnotationsCacheKey(
						that.vars.currentWindowRecording,
					...
					if (arbitrationDoc && !!arbitrationRoundNumber) {
						...
						const uniqueLabels = new Set(
							arbitrationAnnotations.map(
								(a) => a.annotation.value.label
							)
						);
						const hasDisagreement = uniqueLabels.size > 1;
						arbitrationAnnotations.forEach(
							(arbitrationAnnotation) => {
								if (
									arbitrationAnnotation.annotation.user ==
									Meteor.userId()
								)
									return;
								const label =
									arbitrationAnnotation.annotation.value
										.label;
						...
						if (
							hasDisagreement &&
							arbitrationDoc.currentRoundNumber > 1
						) {
							that._showInfoPanel();
						} else {
							that._hideInfoPanel();
